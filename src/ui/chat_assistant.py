...
def build_dashboard_context(result) -> str:
    ...
    if tx is not None and not tx.empty:
        ...
        sample = tx.head(12)
        cols = [c for c in ("date", "merchant", "amount", "source") if c in sample.columns]
        if cols:
            ...
            for _, row in sample.iterrows():
                ...
                amt = row.get("amount", "")
                lines.append(
                    f"  - {date} | {row.get('merchant', '')} | ${float(amt):,.2f} | {row.get('source', '')}"
                )
    ...
    try:
        ...
        if "429" in str(exc) or "quota" in str(exc).lower():
            ...
            try:
                ...
                return (response.text or "").strip() or "Rate limited - try again in a minute."
            except Exception as exc2:
                return f"Gemini is temporarily unavailable ({exc2}). Please try again shortly."
